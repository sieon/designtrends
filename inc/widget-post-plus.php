...
/* ------------------------------------ */
	function __construct() {
      ...
      array(
        ...
        'classname' => 'widget widget-lean-posts'
      )
    );
	}
...
?>
	...
	<?php } elseif( $instance['posts_style'] == 1 ) { ?>
		...
		<ul class="list-unstyled">
			<?php while ($posts->have_posts()): $posts->the_post(); ?>
				<li>
					...
					<small class="text-muted"><?php the_time('j M, Y'); ?></small>
				</li><!--./li-->
			<?php endwhile; ?>
			<?php wp_reset_postdata(); ?>
		</ul><!--./ul-->
...
?>
	...
	<div class="alx-options-posts">
		...
		<p style="padding-top: 0.3em;">
			<label style="width: 100%; display: inline-block;" for="<?php echo esc_attr( $this->get_field_id("posts_style") ); ?>">列表类型：</label>
			<select style="width: 100%;" id="<?php echo esc_attr( $this->get_field_id("posts_style") ); ?>" name="<?php echo esc_attr( $this->get_field_name("posts_style") ); ?>">
			  <option value="0"<?php selected( $instance["posts_style"], "0" ); ?>>上下图文</option>
			  <option value="1"<?php selected( $instance["posts_style"], "1" ); ?>>左右图文</option>
			  <option value="2"<?php selected( $instance["posts_style"], "2" ); ?>>左右小图文</option>
			  <option value="3"<?php selected( $instance["posts_style"], "3" ); ?>>标题+时间</option>
			  <option value="4"<?php selected( $instance["posts_style"], "4" ); ?>>纯标题</option>
			</select>
		</p>
